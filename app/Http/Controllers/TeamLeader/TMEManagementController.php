...
//dd($role_id['0']['assigned_employee_id']);
    ...
    public function TMEleadsList($tmeid){
        // $lead_list = register::join('assigned_leads','assigned_leads.lead_id','register.index_id')->where('assigned_leads.tme_id', $tmeid)->groupBy('register.index_id')->get();
        $leadlist_desc = register::select(DB::raw('max(assigned_leads.id)'))->join('assigned_leads','assigned_leads.lead_id','register.index_id')
        ->where('assigned_leads.tme_id', $tmeid)
        ...
        ->get();
        //dd($lead_list);
        return view('team_leader.list_leads_tme',['lead_list'=>$lead_list]);
    }

    public function TMEParticularleadsList(){
        $userId = Session::get('sales_employee_id');
        $leadlist_desc = register::select(DB::raw('max(assigned_leads.id)'))->join('assigned_leads','assigned_leads.lead_id','register.index_id')
        ->where('assigned_leads.tme_id', $userId)
        ->orderby('assigned_leads.id')
        ->groupBy('assigned_leads.lead_id')->get();

        $leadlist = register::select('register.firstname','register.email','register.birthdate','register.mobile','assigned_leads.id','assigned_leads.lead_id','assigned_leads.status')
        ->join('assigned_leads','assigned_leads.lead_id','register.index_id')
        ->whereIn('assigned_leads.id', $leadlist_desc)
        ->get();
    ...
    public function UpdateLead(Request $request){
         $userId = Session::get('sales_employee_id');
         $lead_list_details = Assignedleads::where('lead_id',$request->lead_id)->first();         
                $assignlead['lead_id'] = $request->lead_id;
                $assignlead['tme_id'] = $lead_list_details->tme_id;
                $assignlead['tl_id'] = $lead_list_details->tl_id;
